...
export const TrendIndicator: React.FC<TrendIndicatorProps> = ({ trend, value, percent, showValue = true }) => {
  const formatVal = (v: number) => Math.abs(v).toFixed(2);
  const formatPercent = (v: number) => Math.abs(v).toFixed(2);

  if (trend === Trend.UP) {
    return (
      <div className="flex items-center text-trend-up text-xs font-semibold tabular-nums">
        <ArrowUp className="w-3 h-3 mr-1" />
        {showValue && <span>+{formatVal(value)} ({formatPercent(percent)}%)</span>}
      </div>
    );
  }
